...
`JSX` -> `React.createElement` -> gives `object` -> react convert that object into `HTML` -> and put into   `DOM`
...
### 4. `{ TitleComponent }` vs `{ <TitleComponent /> }` vs `{ <TitleComponent> </TitleComponent> }` in JSX    
   ...
   [Attribute typing](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/script#attr-type)
